from networksecurity.entity.artifact_entity import DataIngestionArtifact, DataIngestionArtifact
...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not status:
                error_message=f"train data does not contain all columns. required columns: {len(self._schema_config['columns'])} present columns : {len(train_df.columns)}"
                raise Exception(error_message)
            status=self.validate_number_of_columns(test_df)
            if not status:
                error_message=f"test data does not contain all columns. required columns: {len(self._schema_config['columns'])} present columns : {len(test_df.columns)}"
                raise Exception(error_message)
